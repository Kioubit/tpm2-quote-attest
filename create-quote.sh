...
mkdir data/
echo "nonce" > data/quote.nonce

#ECDSA with transient objects
tpm2_createek -c ek.handle -G ecc -u "data/ek.pub"
tpm2_createak -C ek.handle -G ecc -g sha256 -s ecdsa --ak-context "data/ak.ctx" --public "data/ak_public.pem" --format "pem"
tpm2_quote --key-context "data/ak.ctx" --pcr-list sha256:1,2,3,4,5,6,7,8,9 --qualification "data/quote.nonce" --message "data/quote.out" --signature "data/quote.sig" --pcr "data/quote.pcr" --pcrs_format=values --format=plain
